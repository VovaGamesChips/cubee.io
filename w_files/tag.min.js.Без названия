...
function(b){aipAPItag.log("Rewarded AD: Video completed")}.bind(a)),googletag.pubads().addEventListener("rewardedSlotClosed",function(b){aipAPItag.log("Rewarded AD: Closed");this.aipRewardComplete("closed",!1)}.bind(a)));aipAPItag.inIframe()?(aipAPItag.log("iframe rewarded AD, GS=Yes"),googletag.pubads().setTargeting("GS","Yes")):(aipAPItag.log("none iframe rewarded Ad, GS=No"),googletag.pubads().setTargeting("GS","No"));aipAPItag.adUnitsRefreshed.rewarded||(aipAPItag.adUnitsRefreshed.rewarded=0);
aipAPItag.adUnitsRefreshed.rewarded++;var c=aipAPItag.adUnitsRefreshed.rewarded.toString();googletag.pubads().setTargeting("FC",c);aipAPItag.log("Rewarded, FC="+c);aipAPItag.aipAPIrewardedSlot=googletag.defineOutOfPageSlot(aipAPItag.aipAPIrewardedSlotId,googletag.enums.OutOfPageFormat.REWARDED).addService(googletag.pubads());googletag.enableServices();googletag.pubads().refresh([aipAPItag.aipAPIrewardedSlot]);aipAPItag.log("Rewarded AD: called")})};
...
function g(h){var q="",t="",x="";try{q=h.getAd().getContentType(),t=h.getAd().getMinSuggestedDuration(),x=h.getAd().getCreativeId()}catch(E){l(E)}z("CONTENT_PAUSE_REQUESTED",{contentType:q,creativeId:x,minSuggestedDuration:t})}function m(h){y();b.style.display="block";a.showLoading(!1);a.aipConfig.AIP_STARTED&&a.aipConfig.AIP_STARTED();try{aipAPItag.forceRedraw(document.getElementById(a.aipConfig.PREROLL_ELEM_ID))}catch(q){aipAPItag.logError(q)}if(d)try{z("MediaUrl: "+h.getAd().getMediaUrl()+"; AdvertiserName: "+
h.getAd().getAdvertiserName()+"; Description: "+h.getAd().getDescription()+"; Title: "+h.getAd().getTitle())}catch(q){l(q)}z("STARTED");aipAPItag.sendEvent("video-ad-started");aiptag.settings.iSkipButtonTimeout&&(aipAPItag.showSkipButtonTimer=setTimeout(function(){a.showSkipButton();try{document.getElementById("aipVideoAdUiSkipButton").onclick=function(){C.contentComplete();p.destroy();z("CONTENT_RESUME_REQUESTED");D=!1;w("video-ad-completed");aipAPItag.sendEvent("video-ad-removed")}}catch(q){aipAPItag.logError(q)}},
...
(function(){var h=new google.ima.AdDisplayContainer(b);h.initialize();C=new google.ima.AdsLoader(h);C.getSettings().setVpaidMode(google.ima.ImaSdkSettings.VpaidMode.INSECURE);C.addEventListener(google.ima.AdsManagerLoadedEvent.Type.ADS_MANAGER_LOADED,function(q){function t(B){return function(){"COMPLETE"==B&&(aipAPItag.sendEvent("video-ad-completed"),aipAPItag.completedTimeoutPreroll=setTimeout(r,1500));"CLICK"==B&&aipAPItag.sendEvent("video-ad-clicked");"USER_CLOSE"==B&&aipAPItag.sendEvent("video-ad-close");
"LOADED"==B&&(aipAPItag.sendEvent("video-ad-loaded"),aipAPItag.prerollSkipped=!1);"SKIPPED"==B&&(aipAPItag.sendEvent("video-ad-skipped"),aipAPItag.prerollSkipped=!0);if("SKIPPABLE_STATE_CHANGED"==B&&aiptag.settings.iSkipButtonTimeout)try{clearTimeout(aipAPItag.showSkipButtonTimer),document.getElementById("aipVideoAdUiSkipButton").style.display="none"}catch(I){aipAPItag.logError(I)}z(B);aipAPItag.log("Preroll status: "+B)}}if(!G){var x=new google.ima.AdsRenderingSettings;x.loadVideoTimeout=12E3;p=
q.getAdsManager(c,x);p.addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR,u);p.addEventListener(google.ima.AdEvent.Type.CONTENT_PAUSE_REQUESTED,g);p.addEventListener(google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED,r);p.addEventListener(google.ima.AdEvent.Type.STARTED,m);p.addEventListener(google.ima.AdEvent.Type.SKIPPED,t("SKIPPED"));p.addEventListener(google.ima.AdEvent.Type.CLICK,t("CLICK"));p.addEventListener(google.ima.AdEvent.Type.PAUSED,e);p.addEventListener(google.ima.AdEvent.Type.RESUMED,
n);p.addEventListener(google.ima.AdEvent.Type.FIRST_QUARTILE,t("FIRST_QUARTILE"));p.addEventListener(google.ima.AdEvent.Type.MIDPOINT,t("MIDPOINT"));p.addEventListener(google.ima.AdEvent.Type.THIRD_QUARTILE,t("THIRD_QUARTILE"));p.addEventListener(google.ima.AdEvent.Type.LOADED,t("LOADED"));p.addEventListener(google.ima.AdEvent.Type.COMPLETE,t("COMPLETE"));p.addEventListener(google.ima.AdEvent.Type.USER_CLOSE,t("USER_CLOSE"));p.addEventListener(google.ima.AdEvent.Type.SKIPPABLE_STATE_CHANGED,t("SKIPPABLE_STATE_CHANGED"));
try{var E=A();a.aipConfig.AD_FULLSCREEN||a.aipConfig.AD_FILL?p.init(E.adWidth,E.adHeight,google.ima.ViewMode.FULLSCREEN):p.init(E.adWidth,E.adHeight,google.ima.ViewMode.NORMAL);p.start()}catch(B){l(B)}}});C.addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR,u)})();window.addEventListener("resize",function(){if(p&&D)if(a.aipConfig.AD_FULLSCREEN){var h=document.documentElement.clientWidth||document.body.clientWidth||window.innerWidth,q=document.documentElement.clientHeight||document.body.clientHeight||
window.innerHeight;aiptag.settings.iHideBranding||(q-=26);p.resize(h,q,google.ima.ViewMode.FULLSCREEN)}else a.aipConfig.AD_FILL&&(h=a.aipConfig.PREROLL_CONTAINER.clientWidth,q=a.aipConfig.PREROLL_CONTAINER.clientHeight,aiptag.settings.iHideBranding||(q-=26),p.resize(h,q,google.ima.ViewMode.FULLSCREEN))});this.requestAds=function(h,q){z=q;H=(new Date).getTime();var t=new google.ima.AdsRequest;t.adTagUrl=h;var x=A();t.linearAdSlotWidth=x.adWidth;t.linearAdSlotHeight=x.adHeight;t.nonLinearAdSlotWidth=
x.adWidth;t.nonLinearAdSlotHeight=x.adHeight;t.forceNonLinearFullSlot=!0;t.setAdWillAutoPlay(!1);t.setAdWillPlayMuted(!1);C.requestAds(t);F=setTimeout(v,15E3);G=!1;D=!0}}aipAPItag=new aipAPI;aipAPItag.cutomSizeConfig();aipAPItag.initGPT();aipAPItag.setCurrency();aipAPItag.detectBlockedAds();aipAPItag.initGDPR();